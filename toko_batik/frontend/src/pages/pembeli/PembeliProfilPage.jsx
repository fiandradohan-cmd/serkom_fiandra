...
function PembeliProfilPage() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                setLoading(true);
                setError("");
                const response = await pembeliApi.getMe();
    ...
    const handleFileChange = async (e) => {
        ...
        try {
            setUploading(true);
            setError("");
            const response = await pembeliApi.uploadGambar(file);
            const data = response.data || response;
            const filename = data.gambar || data.filename || data.file || data.path;
            if (!filename) throw new Error("Upload gagal: nama file tidak ditemukan.");
        ...
        try {
            ...
            if (session?.user) {
                saveSession(session.token, {
                    ...session.user,
                    nama_d: form.nama_d || session.user.nama_d,
                    uname: form.uname || session.user.uname,
                    foto: form.foto || session.user.foto,
                });
            }
...
export default PembeliProfilPage;
